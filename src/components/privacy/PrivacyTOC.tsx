...
import { PRIVACY_TOC_ITEMS } from '../../data/privacyPolicyContent'
...
export function PrivacyTOCMobile({ activeId, onNavigate }: PrivacyTOCProps) {
  return (
    <nav
    ...
    >
      <div className="max-w-7xl mx-auto px-6 sm:px-8 py-3">
        ...
        <div className="flex gap-2 overflow-x-auto hide-scrollbar touch-pan-x pb-1">
          {PRIVACY_TOC_ITEMS.map(({ id, title }) => (
            <a
              key={id}
              href={`#${id}`}
              onClick={(e) => {
...
export function PrivacyTOCDesktop({ activeId, onNavigate }: PrivacyTOCProps) {
  return (
    <nav aria-label="Privacy policy table of contents" className="sticky top-28">
